...
func (c *Collection) GetDocumentsList(param ...string) string {
	...
	for i, it := range items {
		...
		sb.WriteString(fmt.Sprintf("%d%s", it.id, it.info))
	}
	return sb.String()
}

func (c *Collection) getDocuments(count int, key string) []item {
	c.mu.RLock()
	defer c.mu.RUnlock()
	items := make([]item, 0, len(c.Documents))
	for _, d := range c.Documents {
		info := ""
		if key != "" {
		...
		items = append(items, item{id: id, info: info})
		count--
		if count == 0 {
			break
		}
