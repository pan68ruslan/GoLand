...
func (s *Server) HandleConnection(conn net.Conn) {
	...
	for netScanner.Scan() {
		...
		if len(ll) == 3 {
			switch ll[0] { // the collections level
			...
			default:
				...
				if coll, success := s.store.GetCollection(ll[2]); success {
					...
					switch ll[0] { // the documents level
					...
					case cmd.DocumentsListCmd:
						list := currCollection.GetDocumentsList(ll[1], "owner")
						response = fmt.Sprintf("%s from collection %s", list, collectionName)
					default:
